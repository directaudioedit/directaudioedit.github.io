...
class TangoWrapper(PipelineWrapper):
    def __init__(self, *args, **kwargs) -> None:
        ...
        main_config['unet_model_config_path'] = os.path.join('code/tango', main_config['unet_model_config_path'])
    ...
    def decode_to_mel(self, x: torch.Tensor):
        audio = self.vae.decode_to_waveform(x)
        return torch.from_numpy(audio).float() / 32768.0
...
def _import_tango2_modules():
    from tango.tango2.models import AudioDiffusion as Tango2AudioDiffusion
    from tango.tango2.audioldm.audio.stft import TacotronSTFT as Tango2TacotronSTFT
    from tango.tango2.audioldm.variational_autoencoder import AutoencoderKL as Tango2AutoencoderKL

    return Tango2TacotronSTFT, Tango2AutoencoderKL, Tango2AudioDiffusion


class Tango2Wrapper(PipelineWrapper):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        Tango2TacotronSTFT, Tango2AutoencoderKL, Tango2AudioDiffusion = _import_tango2_modules()

        path = snapshot_download(repo_id=self.model_id)
        vae_config = json.load(open("{}/vae_config.json".format(path)))
        stft_config = json.load(open("{}/stft_config.json".format(path)))
        main_config = json.load(open("{}/main_config.json".format(path)))
        main_config["unet_model_config_path"] = os.path.join("code/tango/tango2", main_config["unet_model_config_path"])

        self.vae = Tango2AutoencoderKL(**vae_config).to(self.device)
        self.stft = Tango2TacotronSTFT(**stft_config)
        self.model = Tango2AudioDiffusion(**main_config).to(self.device)
    ...
    def encode_text_2(self, prompts: List[str]):
        if len(prompts) > 1:
            raise NotImplementedError

        prompt_embeds, boolean_prompt_mask = self.model.encode_text_classifier_free(
            prompts,
...
def load_model(model_id, device, num_diffusion_steps, double_precision=False):
    if 'tango2' in model_id:
        ldm_stable = Tango2Wrapper(model_id=model_id, device=device, double_precision=double_precision)
