...
def next_step(ldm_model, model_output: Union[torch.FloatTensor, np.ndarray],
    ...
    if ldm_model.model.scheduler.config.prediction_type == "v_prediction":
...
@torch.no_grad()
def ddim_inversion(
    ldm_model, w0, prompts, cfg_scale, num_inference_steps, skip
):
    _, text_emb, uncond_emb = get_text_embeddings(prompts, [""], ldm_model)
    latent = w0.clone().detach()

    timesteps = ldm_model.model.scheduler.timesteps

    for i in tqdm(range(num_inference_steps)):
        if num_inference_steps - i <= skip: break
