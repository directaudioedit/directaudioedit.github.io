...
def get_text_embeddings(target_prompt: List[str], target_neg_prompt: List[str], ldm_stable: PipelineWrapper
    ...
    if 'audioldm2' in ldm_stable.model_id:
        text_embeddings_hidden_states, text_embeddings_class_labels, text_embeddings_boolean_prompt_mask = \
            ldm_stable.encode_text_2(target_prompt)
        
        text_emb = PromptEmbeddings(embedding_hidden_states=text_embeddings_hidden_states[1:],
                                    boolean_prompt_mask=text_embeddings_boolean_prompt_mask[1:],
                                    embedding_class_lables=text_embeddings_class_labels[1:])
        uncond_emb = PromptEmbeddings(embedding_hidden_states=text_embeddings_hidden_states[:1],
                                      boolean_prompt_mask=text_embeddings_boolean_prompt_mask[:1],
                                      embedding_class_lables=text_embeddings_class_labels[:1])
